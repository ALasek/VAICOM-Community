...
- Numeric ranges and alternatives match the upstream profile. Both spoken number words and digit transcripts are normalized. The upstream demonstration-only `New Command` entry is intentionally omitted, and Chatter requires the exact phrase `chatter` rather than VoiceAttack's broad `*Chatter*` wildcard.
